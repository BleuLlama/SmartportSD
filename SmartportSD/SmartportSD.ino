...
Disk Drive connector (20 pin)
    ...
    This is the view when looking at the pins on a drive
    The alignment tab is at the top of this diagram
        ...
        WPT WR  RD  DEN SEL WRQ  PH3 PH2 PH1 PH0
...
#define NEW_ROTATE_BOOT 1

#ifdef NEW_ROTATE_BOOT
int rotate_boot (void)
{
  unsigned char np;
  const unsigned char leds[4] = { kLED_D0, kLED_D1, kLED_D2, kLED_D3 };
  unsigned char op[ 4 ];

  op[0] = devices[ 0 ].sdf.isOpen();
  op[1] = devices[ 1 ].sdf.isOpen();
  op[2] = devices[ 2 ].sdf.isOpen();
  ...
  while( 1 )
  {
    if( digitalRead(kEjectPin) == LOW ) {
      ...
      np = (initPartition + 1) %4; 
      while( !devices[ np ].sdf.isOpen() && np != initPartition ) {
        np = (np + 1) %4; 
      }

      // if we looped around, there are no drives available
      // show superfast flashing loop
      while( np == initPartition ) {
        drive_leds( 0 );
    ...
    // make sure open ones are lit appropriately.
    digitalWrite( leds[ 0 ], op[0] );
    digitalWrite( leds[ 1 ], op[1] );
...
//  flashes the drive lights also, with the error code
//
//*****************************************************************************
...
void mcuInit(void)
{
  ...
  // B1 needs to be an output (LED on D9)
  DDRB = DDRB | B00000010;   // 2023-07 SL
